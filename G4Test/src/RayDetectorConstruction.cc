...
G4LogicalVolume* RayDetectorConstruction::CellConstruction()
{
    ...
    G4Sphere* solidCell =
          new G4Sphere("cellSolid", 0, 1770.*cm, 0, 2*pi, 0, pi);
...
{
    G4Sphere* solidDet = 
        new G4Sphere("detSolid", 1800*cm, 1801*cm, 0, 2*pi, 0, pi);

    //G4Box* solidHole = 
    //    new G4Box("holeSolid", 1*mm, 1*mm, 2*cm);

    //G4VSolid* solidSub = 
    //    new G4SubtractionSolid("subSolid", solidDet, solidHole, 0, G4ThreeVector(0, 0, 10*cm));
...
G4VPhysicalVolume* RayDetectorConstruction::DefineVolumes()
{
    ...
    G4VisAttributes* detVisAtt = new G4VisAttributes(G4Colour(0.5, 0.5, 0));
